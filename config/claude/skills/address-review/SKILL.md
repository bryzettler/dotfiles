...
5. **Defer** — one ticket per **defer** in `.scratch/<feature>/issues/` when a folder matches the branch, else `.scratch/pr-<n>-followups/issues/`, in the ticket grammar from `implement-tickets/briefs.md`: `**Status:** ready-for-agent`, a body that quotes the finding and the verifier's scenario, and one acceptance criterion. Done when every **defer** has a ticket path.

6. **Draft and gate** — write `<scratchpad>/replies.json`, one entry per finding that is not **ask**: the reply target from `findings.md` and a body that starts with `<!-- address-review -->`. Each body opens by restating the finding in a few words, so the reviewer sees it was read, then says what the table's reply column says, to the Public text standard. Findings from review bodies and PR comments have no thread: fold them into one PR comment that quotes each finding's first line and answers it. A ticket path is private: a **defer** reply says "tracked as a follow-up" and names no path.

   Then show the user: one row per finding (id, reviewer, `file:line`, verdict, one-line reason), each **ask** with the one question that settles it, the commit sha and its lint and test results, the ticket paths, and the reply bodies. Ask with `AskUserQuestion`: post all, change verdicts first, or stop here. A changed verdict loops back to step 4 or 5 for that finding only. Done when the user picks post or stop. Stop leaves the commit and tickets local and posts nothing.

7. **Push and post** — `git push` (never force). Then post each thread reply with `gh api -X POST repos/{owner}/{repo}/pulls/<n>/comments/<top comment id>/replies -F body=@<file>`, the folded comment with `gh pr comment <n> --body-file <file>`, and a **claim** on the PR description with `gh pr edit <n> --body-file`. Threads stay unresolved; the reviewer resolves them after checking the fix. Done when every reply returns a URL.

## Rerun

A thread whose newest comment carries `<!-- address-review -->` is answered, and the scout skips it; a reviewer reply after that comment reopens it. A rerun after the user stopped at the gate finds the earlier commit ahead of the remote head; the verifiers return **FIXED** with its sha for the findings it covers, so nothing is fixed twice.
