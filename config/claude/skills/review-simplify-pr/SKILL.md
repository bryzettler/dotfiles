---
name: review-simplify-pr
description: Review a PR on four axes (standards, spec, defects, value) and post the surviving findings as inline PR comments with suggestion blocks. A clean PR with no open value suspicion gets an approval. The branch is left untouched.
disable-model-invocation: true
---

# review-simplify-pr

The PR-facing sibling of `review-simplify`: same four axes, same triage, but the output is a GitHub review on the PR, one inline comment per finding, each carrying a `suggestion` block the author can apply with one click. A clean PR gets an approval and silence. Read `/Users/bry/.claude/skills/review-simplify/review-core.md` first: it holds the Defects and Value briefs and lenses, the tooling step, the triage rules, the fix re-review, and the report shape.

The target is a PR number given as the argument; with no argument, the PR open for the current branch (`gh pr view --json number`).

## Steps

1. **Pin** — read `gh pr view <n> --json baseRefName,headRefOid,body`. Review from a checkout at the PR head: the current worktree when `HEAD` is that sha, else a throwaway worktree in the scratchpad from `git fetch origin pull/<n>/head`. Fetch the base and take `origin/<base>` as the fixed point. Write the spec to a scratchpad file: the PR body, then the full commit messages from `git log origin/<base>..HEAD`. The PR body is the strongest spec a PR has, since every sentence in it is a claim the Spec and Contract lenses can test. Run the tooling from `review-core.md` in the checkout and save the output to the scratchpad. Done when the checkout is at `headRefOid`, the fixed point resolves, the spec file exists, and the tooling output is saved or each tool is noted as unavailable.

2. **Review** — from the checkout, invoke `mattpocock-skills:code-review` with `origin/<base>` as the fixed point and the spec file as its spec argument. Dispatch the Defects and Value sub-agents from `review-core.md` in the same batch as code-review's two. Done when four reports are in hand.
...
4. **Gate** — zero confirmed findings and zero open Value suspicions: submit `gh api repos/{owner}/{repo}/pulls/<n>/reviews --input <file>` with `{ "event": "APPROVE", "body": "<!-- review-simplify-pr -->\nLGTM" }`, write the report, and stop. Silence is the deliverable for a clean PR. Zero confirmed findings but one or more open Value suspicions: no approval. Submit a `COMMENT` review whose body asks the one question per suspicion that would close it, written to the "Public text" standard, then write the report and stop.
...
5. **Draft** — one `fixer` run in the checkout. It edits the code as if fixing it, runs the project's lint and tests, then converts each change into a suggestion anchored to the PR diff. Its deliverable is a JSON file in the scratchpad, one entry per comment:
   ...
   Every entry defaults to a `suggestion` fence; the delegation never pre-labels an item as a plain comment. A plain comment is the exception, reached only through the rules under "Anchoring suggestions", and the JSON carries `"plain_reason"` on any entry without a fence so the main loop can check each one before posting. An entry is anchored where the fix lands, not where the symptom shows: if the finding was spotted at one diff line but the edit belongs on another `+` line, the suggestion goes on the latter. A "leave as is" resolution posts nothing; its reasoning lives in the terminal report.

   `fixer` starts with no context and cannot ask, so every judgement call is settled before dispatch and the prompt states the change, not the reasoning. Per item: file and line, what is wrong, and the specific minimal edit. Include the PR head sha, the base branch, the checkout path, the lint and test commands, the JSON path and shape, the anchoring rules, and the "Public text" standard for comment bodies. Done when every item has an entry and lint and tests pass on the edited checkout, or a failure is shown to pre-exist at the PR head.
...
The shape in `review-core.md`, plus: the review URL and whether it was an approval or a comment review, and per posted comment `file:line`, a one-line summary, and `suggestion` or `plain: <reason>`.
