...
The scout runs whichever of these the repo supports, one output file per tool in the scratchpad: `cargo clippy --all-targets`, `gitleaks detect` or `trufflehog filesystem` on the diff, and `squawk` on changed `.sql` files. Then the tooling of each domain file. `cargo audit` runs only when the diff changes a `Cargo.toml` or `Cargo.lock`, and `npm audit` (or `pnpm audit`) only when it changes a `package.json` or a JS lockfile, in any domain: on an unchanged dependency tree an audit repeats the base's result, and a workspace audit can take minutes. Then, in PR mode, `gh pr checks <n>`.

**Test runs.** When there is no CI or no check runs the tests, and always in branch mode, the gate needs one run of the project's test command, plus the test tooling a domain file names (`anchor test`, `forge test`). The scout does not run them. It returns each command with its working directory and an output path, and the main loop starts each one in the background (`run_in_background`) in the same message that dispatches step 2, so the test run overlaps the review instead of delaying it. The main loop reads only the tail of each output file, at the gate. That run is the gate's execution evidence, and the only test run the review pays for: the fixer proves its own edits with narrower commands.

Everything else here is deterministic and near free. Every review prompt carries the paths. Record in the report which ran and which were unavailable or skipped, from the scout's manifest.
...
**The spec is a claim, not a verdict.** A wrong outcome traced to code that does what the spec says is a confirmed finding against the spec. It resolves as "fix the spec", never as dismissed.

**Pre-existing is a finding when the diff leans on it.** A flaw on unchanged lines is confirmed when the diff calls it, widens who reaches it, or makes an outcome depend on it. It is dismissed as pre-existing only with the `file:line` that shows no path in the diff reaches it.

**Revert check.** Reading a test is not running it. One more verifier takes every guard the diff adds (a `require!`, a `revert`, a thrown error, an early return, a validation branch) that a unit test file covering it can check in under a minute, up to ten per round. It works in a throwaway worktree at the reviewed head (in branch mode with the uncommitted changes applied), and per guard: flips or deletes it, runs that one test file, records red or green, and restores it. A guard that stays green is a confirmed Pinned finding with the missing test as its fix. It returns one line per guard: `file:line`, the test file, red or green, and the output path.
...
- **Fix the spec** — when the code follows the spec and the spec is wrong. The code stays as it is, the fixer gets no item, and the report leads with the finding, the spec line, and the one question the user answers to settle it.
...
A severity table first: one row per confirmed finding with file, axis or lens, severity. Then five lists: fixed in code, fixed in the claim, left as is with the reasoning, open suspicions (Value only, each with what was checked and what was not, and whether it was carried from a prior round), dismissed with the reason. Then the execution evidence: CI checks per name as pass, fail, pending, or "no CI"; the test runs as pass or fail with the failing lines when any; then lint and test results per command from the fixer's return, with the failing lines when any, and the entries marked unproven. Then the spec questions from every "fix the spec" resolution. Then the revert check: guards flipped, red, green. Then a coverage line: the head sha reviewed and the base, round (full or delta since `<last>`), domains, files read, files not read, axes skipped, tools run, tools unavailable. On the zero path, the open suspicions and dismissed lists, the execution evidence, the revert check, and the coverage line.
